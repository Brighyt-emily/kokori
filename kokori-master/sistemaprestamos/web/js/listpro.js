...
function listarRegisProfesores(){
    $.get("pc",{"op":1}, function (datos) {
        ...
        + data[i].apellidos+
        "</td><td><form action='InformProfesor.jsp' method='POST'><input type='text' value='"+ data[i].idProfesor +"' name='idProfesor' hidden><button class='btn-floating red' onclick='verProfesor(" + data[i].idProfesor + ");'><i class='material-icons'>portrait</i></button></td><td><button  class='btn-floating' onclick='eliminar(" + data[i].idProfesor + ");'><i class='material-icons'>delete</i></button></td></tr></tr>");                      
        }
    });
}
